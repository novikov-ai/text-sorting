...
If you want change it size provide integer value in MB or enter any key.");
            ...
            var filePath =
                $@"..{dirSeparator}..{dirSeparator}..{dirSeparator}..{dirSeparator}TxtFiles{dirSeparator}generated_mb{fileSizeMb}.txt";
            Console.WriteLine($@"Default file path is: {filePath}");

            try
            {
                new SentenceGenerator(filePath).Generate(fileSizeMb);
            }
            catch (Exception e)
            {
